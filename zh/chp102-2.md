...
答案很简单：其实 module_init 是一个宏定义。将akae_init函数别名为init_module。
见kernel代码的 include/linux/init.h 文件（此处不再重点讲解）。

详见 <http://bbs.chinaunix.net/thread-1350305-1-1.html>

在linux下的驱动函数都有module＿init（x）和module_exit(x)在linux内核里都定义成一个宏例如：
